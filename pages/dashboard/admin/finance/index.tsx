...
import AddExpenseFolder from '@/components/admin/finance/add'
import { jwtDecode } from 'jwt-decode'
import FinanceQuery from '@/components/admin/finance/financeQuery'
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={dataStore.value ? `${styles.row}` : `${styles.column}`}>
                {search ? searchData?.getSearchByFolderName.map(({ expFolderID, exFolder, createdAt }: any) => (
                    <FinanceQuery key={expFolderID} exFolder={exFolder} date={createdAt} expFolderID={expFolderID} userID={userID} />
                )) : loading ? "" : data.getAllExpenseFolder.map(({ expFolderID, exFolder, createdAt }: any) => (
                    <FinanceQuery key={expFolderID} exFolder={exFolder} date={createdAt} expFolderID={expFolderID} userID={userID} />
                ))}
            </div>
        </div >
