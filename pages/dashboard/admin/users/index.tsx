...
import { GetSearchByUser, getAllUserQuery } from '@/lib/util/User/user.query'
import AddUser from '@/components/admin/user/add'
import { UserSubscriptions } from '@/lib/util/User/user.subscriptions'
import UsersQuery from '@/components/admin/user/users'
                        ...
                        {search ? searchData?.getSearchByUser.map(({ userID, email, role, salary, createdAt, myProfile }: any) => (
                            myProfile.map(({ fullname, phone, firstname, lastname, birthday }: any) => (
                                salary.map(({ salary: salaries }: any) => (
                                    <UsersQuery key={userID} userID={userID} email={email} role={role} salary={salaries} createdAt={createdAt} fullname={fullname} phone={phone} firstname={firstname} lastname={lastname} birthday={birthday} mUser={userIds} />
                                ))
