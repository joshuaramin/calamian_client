                    ...
                    <div key={userID} className={styles.profile}>
                        ...
                        <div key={myProfile.profileID} className={styles.prof}>
                            <div className={styles.header}>
                                <h2 className={poppins.className}>{myProfile.fullname}</h2>
                            </div>
                            <div className={styles.cons}>
                                <span>Email: {email}</span>
                                <span>Role: {role.toUpperCase()}</span>
                                <span>Phone: {myProfile.phone.replaceAll("+63", "0")}</span>
                                <span>Birthday: {format(new Date(myProfile.birthday), "MMMM dd, yyyy")}</span>
                                <span>Salary:
                                    <span key={salary.salaryID}> {Intl.NumberFormat("en-US", { style: "currency", currency: "PHP" }).format(salary.salary)}</span>
